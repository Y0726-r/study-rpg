...
function drawGachaItem() {
    ...
    // 🎯 チュートリアル制限：Lv.1の間（初回のガチャなど）はレアリティ2までしか出さない
    if (gameData.player.level < 2 && rarity > 2) {
        console.log("🛡️ チュートリアル制限: レアリティを2に引き下げました");
        rarity = (Math.random() < 0.6) ? 1 : 2; // 1か2に振り分ける
    }

    // 🎯 レベル50制限：宝箱が進化する(Lv.50)までは、レアリティ4以上（エピック・レジェンド）を出さない
    if (gameData.player.level < 50 && rarity >= 4) {
        console.log("🛡️ レベル制限: レアリティを3に引き下げました (Lv50未満)");
        // 4以上を引いたら、3か2に振り分け直す
        rarity = (Math.random() < 0.7) ? 3 : 2;
    }
